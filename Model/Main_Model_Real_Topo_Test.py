        ...
        for row in reader:
            ...
            if source not in G:
                ...
                if source in node_positions:
                    nx.set_node_attributes(G, {source: {'pos': node_positions[source]}})
            if destination not in G:
                G.add_node(destination, data=District(destination, *nodes_info[destination]))
                # 设置节点的位置
                if destination in node_positions:
                    nx.set_node_attributes(G, {destination: {'pos': node_positions[destination]}})
...
def balance_traffic(G, paths):
    ...
    print("总抗阻:", total_loss)
    result_df.to_csv('Traffic_Distribution_Result_Multipath_Test.csv', index=False, encoding='GBK')  # 导出为CSV文件
...
def visualize_traffic_network(G):
    ...
    '''# 手动设置 Jianghan、Jiangan 和 Wuchang 三个节点的位置
    pos['Jianghan'] = (0, 0)
    pos['Jiangan'] = (1, 1)
    pos['Wuchang'] = (-1, 1)'''
...
def main():
    csv_file = 'D:/毕业论文/工程项目/Model/Ta_Real_Topo_Test.csv'  # 替换为你的边权重 CSV 文件路径
    flow_csv = 'D:/毕业论文/工程项目/Model/Traffic_Flow_Real_Topo_Test.csv'  # 替换为你的流量 CSV 文件路径
    info_file = 'D:/毕业论文/工程项目/Model/Information_Final.csv'  # 替换为你的节点信息 CSV 文件路径
